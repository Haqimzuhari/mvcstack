<nav class="fixed w-80 top-0 bottom-0 left-0 bg-white p-4 flex flex-col space-y-10">
    <div class="h-14 flex-none">
        <div class="flex items-center space-x-4 px-6">
            <div class="w-14 h-14 rounded-full bg-zinc-100 overflow-hidden">
                <img src="<?=(!auth()->check()) ? 'https://api.dicebear.com/6.x/bottts/svg?seed=' . uniqid() : ((auth()->user()->profile->picture) ? asset('images/profile-pictures/' . auth()->user()->profile->picture) : 'https://api.dicebear.com/6.x/bottts/svg?seed=' . auth()->user()->profile->fullname)?>" class="w-full h-full"/>
            </div>
            
            <div>
                <p class="font-semibold text-xl">Welcome,</p>
                <p class="font-light"><?=auth()->user()->profile->fullname?></p>
            </div>
        </div>
    </div>
    ...
    <div class="h-40 flex-none flex flex-col justify-end">
